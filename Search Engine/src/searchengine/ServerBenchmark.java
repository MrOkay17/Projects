...
public class ServerBenchmark {
    ...
    public void setup() {
        try {
            ...
            while (server == null) {
                try {
                    server = new Server(rnd.nextInt(60000) + 1024,filename);
    ...
    // @Benchmark
    // public Map<String, List<Page>> measureAvgTime() throws InterruptedException, IOException {
    //     // Probably not a good idea to search for the same thing all the time... oh well
    //     Database db = new Database(filename);
    //     Search s = new Search(db.getPages());
    //     return s.searchList("denmark");
    // }
}
